...
function StatusBadge({ status }: { status: IngestRow["status"] }) {
  ...
  // @ts-ignore shadcn variant type
  return <Badge variant={map[status] || "outline"}>{status}</Badge>;
}

export default function OperatorDashboard() {
  const [tab, setTab] = useState<"versions" | "ingestions">("versions");
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-2">
          <Link to="/operator-versions"><Button variant="outline">Manage Versions</Button></Link>
          <Link to="/operator-ingestions"><Button variant="outline">Ingestion Logs</Button></Link>
        </div>
      </div>

      <div className="flex gap-2">
        <Button variant={tab === "versions" ? "default" : "outline"} onClick={() => setTab("versions")}>
          Regulations & Versions
        </Button>
        <Button variant={tab === "ingestions" ? "default" : "outline"} onClick={() => setTab("ingestions")}>
          Ingestion Logs
        </Button>
      </div>
      ...
      {tab === "versions" && (
        <Card className="p-4">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-lg font-semibold mb-1">Manage Regulations & Versions</div>
              <div className="text-sm text-muted-foreground">
                Add/remove versions, re-ingest docs, and view coverage.
              </div>
            </div>
            <Link to="/operator-versions"><Button>Open Versions</Button></Link>
          </div>
        </Card>
      )}
      ...
      {tab === "ingestions" && (
        <Card className="p-4">
          <div className="mb-3 flex items-center justify-between">
            <div className="text-lg font-semibold">Ingestion Logs</div>
          ...
          <div className="grid gap-3">
            ...
            {!ing.length && <div className="text-sm text-muted-foreground">No runs found.</div>}
          </div>
